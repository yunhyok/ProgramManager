...
internal static class ManagerUpdaterChecks
{
    ...
    private static void CheckCombined(string root)
    {
        ...
        var localSearch = (TextBox)typeof(MainForm).GetField("_localSearch", flags)!.GetValue(form)!;
        var catalogSearch = (TextBox)typeof(MainForm).GetField("_catalogSearch", flags)!.GetValue(form)!;
        ...
        LayoutCheck.Descendants(tabs.TabPages[1]).OfType<Button>().Single(b => b.Text == "목록 새로고침").PerformClick();
        WaitFor("_busy");
        Assert(catalogRefreshes == 1 && state.Cache.Catalog.Apps.Single().Id == "example" && catalogSearch.Text == "Example" && localSearch.Text == "local filter", "catalog refresh contacts the host and preserves both filters");
        typeof(MainForm).GetMethod("ShowAppUpdate", flags)!.Invoke(form, new object?[] { "example" });
        Assert(catalogSearch.Text == "" && localSearch.Text == "local filter", "tray navigation clears only the catalog filter");
        ...
        Assert(catalogRefreshes == 2, "actual client refresh requests host GitHub synchronization");

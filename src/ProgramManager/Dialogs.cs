...
internal static class Dialogs
{
    ...
    public static SettingsChange? Settings(IWin32Window owner, AppState state)
    {
        ...
        var host = form.TextField("호스트 주소 / 이름", state.Settings.AdvertisedHost);
        var port = new NumericUpDown { Minimum = 1024, Maximum = 65535, Value = state.Settings.Port };
        form.Row("호스트 수신 포트", port);
        var auto = new CheckBox { Text = "Windows 로그인 시 트레이에서 시작", AutoSize = true, Checked = state.Settings.AutoStart };
        form.Row("시작 옵션", auto);
        ...
        var disconnect = new CheckBox { Text = "기존 호스트 연결 해제", AutoSize = true };
        form.Row("", disconnect);
    ...
    public static void ShowPairing(IWin32Window owner, string code, string address)
    {
        ...
        form.Row("", Ui.Bar(Ui.Button("코드 복사", (_, _) => { Clipboard.SetText(code); }), Ui.Button("닫기", (_, _) => form.Close())));
